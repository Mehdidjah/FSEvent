...
defmodule FileSystem.Backends.FSInotify do
  @moduledoc """
  File system backend for GNU/Linux, FreeBSD, DragonFly and OpenBSD using inotify.

  This backend uses the inotify subsystem available on Linux and BSD systems to monitor
  file system changes. It requires the `inotify-tools` package to be installed.

  ## How It Works

  The backend uses the `inotifywait` command-line tool to monitor directories for changes.
  Events are streamed back to the Elixir process via a Port, where they are parsed
    ...
    if is_nil(exec_file) do
      Logger.error(
        "`inotify-tools` is needed to run `file_system` for your system. " <>
          "Please install it using your system's package manager (e.g., `apt-get install inotify-tools` on Debian/Ubuntu). " <>
          "If it's already installed but not found, specify the executable file path with `config.exs` or `FILESYSTEM_FSINOTIFY_EXECUTABLE_FILE` environment variable."
      )
