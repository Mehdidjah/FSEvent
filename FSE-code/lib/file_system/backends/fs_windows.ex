...
defmodule FileSystem.Backends.FSWindows do
  @moduledoc """
  File system backend for Windows using FileSystemWatcher.

  This backend uses the native Windows FileSystemWatcher API to monitor file system changes.
  The binary executable file is packaged with the application.

  ## How It Works

  The backend spawns a native Windows executable that uses the FileSystemWatcher API to monitor
  specified directories. Events are streamed back to the Elixir process via a Port,
  where they are parsed and forwarded to subscribers.
  ...
  defp executable_path(:system_env) do
    System.get_env("FILESYSTEM_FSWINDOWS_EXECUTABLE_FILE")
  end
